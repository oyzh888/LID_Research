...
exp_name = 'LIDALL_resNet_Cifar10_BS%d_epochs%d' % (batch_size, epochs)
# Model parameter
# ----------------------------------------------------------------------------
...
mask = np.random.choice(len(x_train),len(x_train))
...
for dataset_pic_id in pbar(range(0,len(x_train),lid_batch_size)):
    ...
    z_train_lid.extend(dis)

print('Finish LID')
sorted_indexes = np.argsort(z_train_lid)
x_train = np.asarray(x_train[sorted_indexes])
y_train = np.asarray(y_train[sorted_indexes])
...
for i in range(batch_num):
    for j in range(batch_size):
        ...
        np.append(y_train_new, y_train[i*batch_size + j])
print('Finish Sorted Batches')
...
callbacks = [checkpoint, lr_reducer, lr_scheduler,TensorBoard(
    log_dir='../TB_logdir/LID_10_10/' + exp_name, write_images=False)]
