...
exp_name = 'Gaussie_Cifar10_BS%d_epochs%d_Duplicate1' % (batch_size, epochs)
...
# x_train_ramdom = x_train[mask]
##get pretrain model
pre_model = keras.models.load_model('/unsullied/sharefs/ouyangzhihao/DataRoot/Exp/HTB/LID_Research/saved_models/preTrainedCIFAR10_augX10.h5')

(x_train_ramdom, _), (_, _) = keras.datasets.cifar100.load_data()

# Normalize data.
x_train_ramdom = x_train_ramdom.astype('float32') / 255
# If subtract pixel mean is enabled
if subtract_pixel_mean:
    x_train_ramdom_mean = np.mean(x_train_ramdom, axis=0)
    x_train_ramdom -= x_train_ramdom_mean

size = (50000,32,32,3)
noise = np.random.normal(0,1,size)
x_train_ramdom = noise
new_labels = pre_model.predict(x_train_ramdom, batch_size=1024)
# y_train_random = y_train[mask]

print(new_labels.shape)
print(x_train_ramdom.shape)
print(x_train_ramdom[0])
print(new_labels[:10])
...
if not data_augmentation:
    ...
    model.fit(x_train_ramdom, new_labels,
