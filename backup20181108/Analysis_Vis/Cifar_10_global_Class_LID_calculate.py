...
生成较大按照class分类下batch下的全局LID分布，网络的输入是未经data augmentation的Cifar10数据集。
由于按照class进行分类，因此直接设置了batch size为一个class的全体样本数。
"""
...
# plt.imshow
# Training parameters
batch_size = 5000  # 一个类有5000个样本，按照类对LID进行计算，因此BS=5000
num_classes = 10
...
for i in range(num_classes):
    ...
    print("Class %d has %d samples"%(i,len(x_train_class)))
exp_name='Cifar10_ground_truth_dataset%d_BS%d_Globale_Class_lid_K%d'%(x_train.shape[0],batch_size,lid_k)
from progressbar import *
pbar = ProgressBar()
for i in pbar(range(num_classes)):
    mask_batch=y_train.flatten()==i
    # import ipdb;ipdb.set_trace()
