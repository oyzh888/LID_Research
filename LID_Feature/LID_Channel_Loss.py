...
def LID(x, LID_extract):  # LID_extract:[batch,rows,cols,channels]
    ...
    for i_batch in range(batch_size):  # 第i个数据计算平均LID
        r = tf.reduce_sum(XX[i_batch] * XX[i_batch], 1)
        # turn r into column vector
        r1 = tf.reshape(r, [-1, 1])
        D = r1 - 2 * tf.matmul(XX[i_batch], tf.transpose(XX[i_batch])) + tf.transpose(r1) + \
            tf.ones([32, 32])
        # find the k nearest neighbor.00
        D1 = -tf.sqrt(D)
        D2, _ = tf.nn.top_k(D1, k=10, sorted=True)
